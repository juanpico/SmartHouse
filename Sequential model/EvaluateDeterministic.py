...
for i in tqdm(INSTANCES):
    ...
    for date in date_list:
        ...
        # Initialize policy
        policy = Policies.DeterministicPolicy(house)
    ...
    # Save results
    outdir = 'Final Results/Deterministic/{}'.format(i)
    if not os.path.exists(outdir):
        os.mkdir(outdir)

    #results.to_csv("Final Results/Deterministic/{}/results.csv".format(i))
    #decision_df.to_csv("Final Results/Deterministic/{}/decisions.csv".format(i))
    #param_df.to_csv("Final Results/Deterministic/{}/model_params.csv".format(i))

# Save times
times = times*(1/len(date_list))*(1/len(theta_values))
np.savetxt("Final Results/Deterministic/execution_time.csv", times, delimiter=",")
# %%
